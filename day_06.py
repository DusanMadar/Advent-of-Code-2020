...
def _load_answers_per_group(path: str) -> List[Set[str]]:
    answers = []
    with open(path) as fh:
        for is_blank, lines in groupby(fh, lambda line: line == "\n"):
            if is_blank:
                continue
            answers.append(set("".join([line.rstrip() for line in lines])))

    return answers


def _load_answers_per_group_member(path: str) -> List[List[Set[str]]]:
    answers = []
    with open(path) as fh:
        for is_blank, lines in groupby(fh, lambda line: line == "\n"):
            if is_blank:
                continue
            answers.append([set(line.rstrip()) for line in lines])
...
def puzzle_1():
    per_group_answers = _load_answers_per_group("./day_06_data.txt")
    return sum(len(answers) for answers in per_group_answers)


assert puzzle_1() == 6583
...
# In the third group, everyone answered yes to only 1 question, a. Since some people
# did not answer "yes" to b or c, they don't count.
# In the fourth group, everyone answered yes to only 1 question, a.
# In the fifth group, everyone (all 1 person) answered "yes" to 1 question, b.
# In this example, the sum of these counts is 3 + 0 + 1 + 1 + 1 = 6.

# For each group, count the number of questions to which everyone answered "yes". What
# is the sum of those counts?
def puzzle_2():
    per_group_member_answers = _load_answers_per_group_member("./day_06_data.txt")
    return sum(
        len(member_answers[0].intersection(*member_answers[1:]))
        for member_answers in per_group_member_answers
    )


assert puzzle_2() == 3290
